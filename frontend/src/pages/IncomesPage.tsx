...
export default function IncomesPage() {
  ...
  function openEdit(item: Income) { setEditing(item); setRecurring(Boolean(item.recurrence_id));  setError(''); setShowForm(true); window.requestAnimationFrame(() => document.getElementById('income-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' })) }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      expected_date: expectedDate, received_date: status === 'received' ? editing?.received_date || today() : null,
  ...
  return <>
    <PageHeader title="Rendas" subtitle="Salários, extras e recebimentos recorrentes" actions={<><input className="month-input" type="month" value={month} onChange={(event) => setMonth(event.target.value)} /><button className="primary-button compact" onClick={openNew}>+ Nova renda</button></>} />
