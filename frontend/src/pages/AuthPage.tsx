...
export default function AuthPage({ onAuthenticated }: { onAuthenticated: (user: User) => void }) {
  ...
  return <div className="auth-layout">
    <section className="auth-hero">
      ...
      <h1>Smart Finance</h1>
      <p>Organize rendas, despesas, cartões e empréstimos em um sistema rápido, local e privado.</p>
      <div className="hero-feature"><b>✓</b><span>Dados separados por usuário</span></div>
      <div className="hero-feature"><b>✓</b><span>Relatório mensal em PDF</span></div>
      <div className="hero-feature"><b>✓</b><span>Acesso no computador e celular</span></div>
    </section>
    <section className="auth-panel">
      <div className="auth-card">
        <h2>{mode === 'login' ? 'Entrar' : mode === 'register' ? 'Criar conta' : 'Recuperar senha'}</h2>
        <p>{mode === 'login' ? 'Use seu usuário ou e-mail.' : mode === 'register' ? 'Crie sua conta local.' : 'Use a chave de recuperação criada no cadastro.'}</p>
        <form onSubmit={submit}>
          ...
          {mode === 'register' && <>
            ...
            <label>Chave de recuperação<input name="recovery_key" minLength={6} required /><small>Guarde esta chave fora do sistema.</small></label>
          </>}
          {mode === 'recover' && <>
