...
export default function SettingsPage({ user, onUser }: { user: User; onUser: (user: User) => void }) {
  ...
  return <>
    ...
    <section className="settings-grid">
      ...
      <article className="panel"><h3>{isNativeMobile() ? 'Aplicativo local' : 'Acesso local'}</h3>{isNativeMobile() ? <><p>Os dados deste aplicativo ficam no banco SQLite do celular.</p><p>O app funciona sem computador e sem internet.</p></> : <><p>No computador: <code>http://localhost:8000</code></p><p>Na rede: <code>http://smartfinance.local:8000</code> ou pelo endereço IP.</p></>}</article>
    </section>
