...
import { requestNativeNotificationPermission, scheduleNativeAlerts } from '../services/mobile/notifications'
...
export default function AlertCenter() {
  ...
  useEffect(() => {
    api<AlertItem[]>('/alerts').then((data) => {
      setItems(data)
      if (isNativeMobile()) {
        void scheduleNativeAlerts(data)
  ...
  async function enableNotifications() {
    if (isNativeMobile()) {
      ...
      if (granted) await scheduleNativeAlerts(items)
      return
    }
